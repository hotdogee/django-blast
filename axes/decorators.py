...
def lockout_response(request):
    if LOCKOUT_TEMPLATE:
        context = {
            'title': 'Account locked: too many login attempts', # hotdogee
            'cooloff_time': COOLOFF_TIME,
            'failure_limit': FAILURE_LIMIT,
        }
        if COOLOFF_TIME: # hotdogee
            context['info'] = 'Please try again later.'
        else:
            context['info'] = 'Contact an admin to unlock your account.'
