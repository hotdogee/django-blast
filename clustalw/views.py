...
def create(request, iframe=False):
    ...
    if request.method == 'GET':
        return render(request, 'clustalw/main.html', {
            'title': 'Clustalw Query',
            'iframe': iframe
        })
    elif request.method == 'POST':
        # setup file paths
        task_id = uuid4().hex
        task_dir = path.join(settings.MEDIA_ROOT, 'clustalw', 'task', task_id)
        # file_prefix only for task...
        file_prefix = path.join(settings.MEDIA_ROOT, 'clustalw', 'task', task_id, task_id)
        ...
        if 'query-file' in request.FILES:
            ...
            return render(request, 'clustalw/invalid_query.html', {'title': 'Invalid Query',})
        ...
        if request.POST['sequenceType'] in ['dna', 'protein']:
            ...
            return redirect('clustalw:retrieve', task_id)
        else:
            raise Http404
...
def retrieve(request, task_id='1'):
    ...
    try:
        r = ClustalwQueryRecord.objects.get(task_id=task_id)
        ...
        if r.result_date and (r.result_date.replace(tzinfo=None) >= (datetime.utcnow()+ timedelta(days=-7))):
            file_prefix = path.join("/media", 'clustalw', 'task', task_id, task_id+".aln")

            os.chdir(path.join(settings.MEDIA_ROOT, 'clustalw', 'task', task_id))
            ...
            with open(path.join(settings.MEDIA_ROOT, 'clustalw', 'task', task_id, task_id+".aln"), 'r') as content_file:
            ...
            if r.result_status in set(['SUCCESS',]):
                ...
                return render(request, 'clustalw/results_not_existed.html',
                {
                    'title': 'No Hits Found',
                    'isNoHits': True,
                    'isExpired': False,
...
def status(request, task_id):
    if request.method == 'GET':
        status_file_path = path.join(settings.MEDIA_ROOT, 'clustalw', 'task', task_id, 'status.json')
...
from .serializers import UserClustalwQueryRecordSerializer
...
def user_tasks(request, user_id):
    ...
    """
    if request.method == 'GET':
        records = ClustalwQueryRecord.objects.filter(user__id=user_id)
        serializer = UserClustalwQueryRecordSerializer(records, many=True)
