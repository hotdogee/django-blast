...
def create(request):
    ...
    if request.method == 'GET':
        ...
        chmod(path.dirname(query_filename), stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO) # ensure the standalone dequing process can open files in the directory
        bin_name = 'bin_linux'
        if platform == 'win32':
            bin_name = 'bin_win'
        # write query to file
        ...
        chmod(query_filename, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO) # ensure the standalone dequing process can access the file
        # build blast command
