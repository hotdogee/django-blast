    ...
    if not path.isfile(file_prefix + '.asn'):
        ...
        try:
            ...
            with open(csv_path, 'r') as f:
                hsp_list = [dict(zip(blast_info['col_names'], [convert(value) for convert, value in zip(blast_info['col_types'], row)])) for row in csv.reader(f)]
                ...
                for db_name, db_hsp_list in groupby([hsp for hsp in hsp_list if sseqid_db[hsp['sseqid']] in db_url], lambda hsp: sseqid_db[hsp['sseqid']]).items():
                    with open(path.join(basedir, db_organism[db_name] + '.gff'), 'wb') as fgff:
                        fgff.write('##gff-version 3\n')
                        match_id = 1
                        match_part_id = 1
                        for hsp_list in [sorted(hsps, key=lambda h: (h['sstart'], h['send']) if h['sstrand'] == 'plus' else (h['send'], h['sstart'])) for hsps in groupby(db_hsp_list, lambda x: x['qseqid'] + x['sseqid'] + x['sstrand']).values()]:
